...
public class Main {
    ...
    private static def generate(pb){
        ...
        */
        ...
        def types = [ 9, 10 ]
        institutionID = 34 //Belmont //TODO this is test only because there's a bug in dev branch of REST to get the correct ID
        def institution = getInstitution(institutionList, 3805/*institutionID*/) //TODO remove the hard-coded id
            ...
            surveys.eachWithIndex { survey, pbIndex ->
                ...
                if(reportID) {
                    ...
                    TERM_CODE_MAP.each { t ->
                        if (t.key.equalsIgnoreCase(term)) term_code = "${survey.year}${t.value}" //Year_Term_Code
                    }
                    print "${term_code},"
                    ...
                    */
                    questionList.eachWithIndex{ questionID, index ->
                        ...
                        if (index == 3){
                            ...
                            //Print Obj1-12
                            if (index >= 4 && index <= 15) {
                                print "${objectives[index-4]}," //e.g., objectives[0] = obj1
                            }
                    ...
                    (questionList + DIAGNOSTIC_QUESTION_TEACHING_METHOD_ID_LIST + (DIAGNOSTIC_ONLY_QUESTION_ID_LIST - [486,488])).each{ questionID ->
                        if (verboseOutput) println("questionID= ${questionID}")
    ...
    static def getAllSurveys(institutionID, types, startDate, endDate) {
        ...
        while((totalResults > resultsSeen + currentResults) && (resultsSeen < MAX_SURVEYS)) {
            def response = client.get( //TODO change the path for start /enddate
                path: "${basePath}/surveys", //?&startDate='${startDate}'&endDate='${endDate}'
